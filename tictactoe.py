...
def init_state():
    ...
    'message': {
        'data': [
            ...
            GAME_BOARD_TEMPLATE.format(GAME_PIECE_LOCS[0], 
                                       GAME_PIECE_LOCS[1], 
                                       GAME_PIECE_LOCS[2],
                                       GAME_PIECE_LOCS[3], 
                                       GAME_PIECE_LOCS[4], 
...
def calculate_time_elapsed(state):
    ...
    return str(datetime.timedelta(seconds=round(
        time.time() - state['game_start'])))
...
def find_lowest_empty_square(state, direction, preference):
    ...
    if direction == 'up':
        for i in preference: # priority order for upwards moves
            if (game_piece_array[i] == ' ' and 
                calculate_hovered_square(state) != i):
                return i
    elif direction == 'down':
        for i in preference:
            if (game_piece_array[i] == ' ' and 
...
def construct_game_board(game_piece_locs):
    ...
    return GAME_BOARD_TEMPLATE.format(GAME_PIECE_LOCS[0], 
                                      GAME_PIECE_LOCS[1], 
                                      GAME_PIECE_LOCS[2],
                                      GAME_PIECE_LOCS[3], 
                                      GAME_PIECE_LOCS[4], 
...
def is_tic_tac_toe(player, state):
    ...
    # left to right check
    if (state['game_piece_locs'][0] == player and 
        state['game_piece_locs'][1] == player and 
        state['game_piece_locs'][2] == player):
        return True
    elif (state['game_piece_locs'][3] == player and 
          state['game_piece_locs'][4] == player and 
          state['game_piece_locs'][5] == player):
        return True
    elif (state['game_piece_locs'][6] == player and 
          state['game_piece_locs'][7] == player and 
          state['game_piece_locs'][8] == player):
        return True
    # top to bottom check
    if (state['game_piece_locs'][0] == player and 
        state['game_piece_locs'][3] == player and 
        state['game_piece_locs'][6] == player):
        return True
    elif (state['game_piece_locs'][1] == player and 
          state['game_piece_locs'][4] == player and 
          state['game_piece_locs'][7] == player):
        return True
    elif (state['game_piece_locs'][2] == player and 
          state['game_piece_locs'][5] == player and 
          state['game_piece_locs'][8] == player):
        return True
    # diagonal check
    if (state['game_piece_locs'][0] == player and 
...
def update_state(stdscr, curr_state, c):
    ...
    if c > 0:
        ...
        # TODO(eddie): this routine could be made better/less brittle by making the offsets relative to
                ...
                try:
                    if c == ord(' '): # spacebar keypress
                        if curr_state['game_piece_locs'][index] == ' ':
                            ...
                            free_sq = find_lowest_empty_square(new_state, 
                                                               "right", 
                                                               [0, 1, 2, 3, 4, 5, 6, 7, 8])
                            coord = map_index_to_coordinate(free_sq)
                            new_state['cursor']['x'] = coord[0]
